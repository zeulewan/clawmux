...
export function useKaraokePlayer() {
  ...
  const tick = useCallback(() => {
    ...
    for (let i = 0; i < a.words.length; i++) {
      const w = a.words[i];
      if (elapsed >= w.start_time && elapsed < w.end_time) {
        newIdx = i;
        break;
      }
    }
  ...
  const seek = useCallback(
    (time) => {
      ...
      activeIdx.current = -1;
      try {
        audioRef.current?.source?.onended && (audioRef.current.source.onended = null);
        audioRef.current?.source?.stop();
      } catch {}
  ...
  const play = useCallback(
    async (audio_b64, rawWords, msgId) => {
      stopPlayback();
      ...
      if (ctx.state === 'suspended') {
        try {
          ...
          console.error('[karaoke] ctx.resume failed:', e);
        }
      }
