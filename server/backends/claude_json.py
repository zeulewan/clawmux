...
class ClaudeJsonBackend(AgentBackend):
    ...
    def prepare_workspace(self, work_dir: str) -> None:
        """Accept workspace trust so Claude Code doesn't prompt."""
    ...
    async def _listen(self, proc: asyncio.subprocess.Process, session_name: str) -> None:
        ...
        try:
            while proc.returncode is None:
                ...
                log.info("[%s] Stream event: type=%s", session_name, event_type)
    ...
    async def _handle_stream_event(
        ...
        if event_type == "content_block_start":
            ...
            if block_type == "text":
                text_buffer.clear()
                # New unique msg_id per text block (prevents DOM reuse across turns)
                ...
                if text:
                    ...
                    try:
                        await send_fn({
                            ...
                            "session_id": session_name,
                            "text": accumulated,
                            "msg_id": getattr(self, '_stream_msg_id', f"cj-{session_name}-{uuid.uuid4().hex[:8]}"),
