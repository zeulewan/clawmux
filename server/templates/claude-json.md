...
- Respond with direct text output. Your text is streamed to the user's browser in real time. Use `clawmux send` ONLY for inter-agent messaging (--to <agent>), NOT for speaking to the user.
...
```bash
```
Just write your response as normal text. It will be rendered with markdown formatting and optionally read aloud via TTS.
...
Your text output is streamed directly to the user's browser. Use `clawmux send` only for inter-agent messages (--to <agent_name>).

# Message Delivery
The hub delivers messages to you via stdin at any time — whether you are idle or mid-task. They arrive as new input with a typed prefix:
