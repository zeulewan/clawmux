...
3. The hub sends the `/clawmux` skill to Claude, activating voice mode
...
- **set_project_status** — Update the sidebar to show what project and area the agent is working on (e.g. "clawmux · frontend"). Agents call this on startup and whenever their context changes.
- **voice_chat_status** — Check if a browser is connected. Agents call this on startup to make sure someone is listening.

## The /clawmux Skill

The `/clawmux` skill is a Claude Code slash command that activates voice chat mode. It's sent to each agent automatically when they start up. The skill tells Claude to:
...
See the raw skill: [/clawmux skill](reference/configuration.md#clawmux-skill)
...
The template is generated by `session_manager.py` and written to `/tmp/clawmux-sessions/{voice_id}/CLAUDE.md`.

See the raw template: [CLAUDE.md Template](reference/configuration.md#claudemd-template)
...
**Starting:** Click a voice card → hub creates tmux + working directory + MCP config + CLAUDE.md → Claude starts → MCP server connects → /clawmux skill activates → agent sets project status → agent greets you
...
| `.claude/commands/clawmux.md` | The /clawmux skill that activates voice mode |
