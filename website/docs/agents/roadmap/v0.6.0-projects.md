# v0.6.0 — Project Folders

Multi-project support for ClawMux. Each project gets its own set of 7 agents, chat history, work directories, and context.
...
class Project:
    ...
    created_at: float
    work_dir: str                # /tmp/voice-hub-sessions/{name}/
...
- If no work dir specified, uses `/tmp/voice-hub-sessions/{slug}/`
...
4. Namespace work dirs: `/tmp/voice-hub-sessions/{project}/{voice_id}/`
...
**No auto-migration.** The default project keeps the existing flat directory layout (`/tmp/voice-hub-sessions/{voice_id}/`) forever. Named projects are opt-in and get their own subdirectories (`/tmp/voice-hub-sessions/{project}/{voice_id}/`).
...
```bash
ps aux | grep "python.*hub.py" | grep -v grep  # find PID
kill <PID>
cd /home/zeul/GIT/voice-chat/server
nohup python hub.py > /tmp/voice-hub.log 2>&1 &
```
...
```bash
cd /tmp/voice-hub-sessions/default && mv * ../ && cd .. && rmdir default
```

### 6. If projects.json is corrupted

```bash
rm /home/zeul/GIT/voice-chat/data/projects.json
