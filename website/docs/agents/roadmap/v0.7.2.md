# v0.7.2 — Flat Architecture & Centralized State
...
Each agent gets one directory. Project assignment is metadata, not filesystem structure.

## Data Model: `agents.json`

Centralized file at `data/agents.json`, managed exclusively by the hub. Replaces per-agent `.session.json` and `.project_status.json` files.
...
| Approach | Current (per-agent files) | v0.7.2 (centralized) |
|---|---|---|
