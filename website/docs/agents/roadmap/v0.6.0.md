...
Replace tmux-injection messaging with Claude Code hooks for reliable, event-driven agent communication.

## Problem

The current system injects messages via `tmux send-keys`. This is fragile:

- Special characters corrupt sessions
- Messages lost if agent is mid-output
- No delivery confirmation
- Race conditions with Ink's input handling
- Messages only arrive when the agent calls `clawmux converse`
...
New command replacing `clawmux converse ""` for listening. Watches **both** the microphone (voice input) and the inbox file simultaneously. Whichever arrives first unblocks the call — like `Promise.race()`.
...
The following commands are deprecated in favor of the unified `send`/`wait` model:
...
## Remaining from Previous v0.6.0 Plan

The following items from the original v0.6.0 scope (orchestration, workspace, sub-agents) are deferred to later releases:
...
Project folders shipped in v0.5.x. See [Project Folders Spec](v0.6.0-projects.md) for reference.
