# Install ClawMux — Linux Split

Hub runs locally on a Linux machine (no GPU required). TTS and STT run on a remote GPU server.

**Requirements:** Linux (Ubuntu 20.04+/Debian 11+), Python 3.10+, Claude Code, remote Linux server with NVIDIA GPU accessible via network

## 1. System Check (Local Machine)
...
- **Tailscale** (recommended): Both machines on the same tailnet. Use Tailscale IPs.
  ...
  ```
- **SSH tunnel**: Forward ports from local to GPU server.
  ```bash
  # On local machine — tunnel TTS and STT
